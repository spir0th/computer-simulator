...
func _init_custom_properties():
	...
	Global.application_version = ProjectSettings.get_setting("application/config/version", Engine.get_version_info())
	...
	AudioServer.set_bus_volume_db(AudioServer.get_bus_index("Master"), ProjectSettings.get_setting("audio/buses/volume/master", -4.8))
	AudioServer.output_device = ProjectSettings.get_setting("audio/devices/output_device", "Default")
